...
test('NODE_FS sync runtime matches Node for bytes, encodings, existence and exclusive flags', async () => {
  const nodeSource = [
    ...
    "fs.writeFileSync('ascii.bin', 'é', { encoding: 'ascii' });",
    "console.log(fs.readFileSync('ascii.bin', 'hex'));",
    "fs.writeFileSync('b64.bin', 'aGV!sbG8=', { encoding: 'base64' });",
    "console.log(fs.readFileSync('b64.bin', 'utf8'));",
    "fs.writeFileSync('u16.bin', Buffer.from([0x00, 0xd8, 0x61]));",
    "const u16 = fs.readFileSync('u16.bin', 'utf16le');",
    "console.log(u16.charCodeAt(0).toString(16), u16.length);",
    '',
  ].join('\n');
  const csharpSource = [
    'using System.Text;',
    ...
    'Line(b.IsBuffer.ToString().ToLowerInvariant());',
    ...
    'catch (NodeFsException e) { Line(e.Code); }',
    'NodeFs.WriteFileSync("ascii.bin", "é", NodeFsWriteFileOptions.FromEncoding("ascii"));',
    'Line(NodeFs.ReadFileSync("ascii.bin", NodeFsReadFileOptions.FromEncoding("hex")).Text);',
    'NodeFs.WriteFileSync("b64.bin", "aGV!sbG8=", NodeFsWriteFileOptions.FromEncoding("base64"));',
    'Line(NodeFs.ReadFileSync("b64.bin", NodeFsReadFileOptions.FromEncoding("utf8")).Text);',
    'NodeFs.WriteFileSync("u16.bin", new NodeFsBuffer(new byte[] { 0x00, 0xd8, 0x61 }));',
    'var u16 = NodeFs.ReadFileSync("u16.bin", NodeFsReadFileOptions.FromEncoding("utf16le")).Text;',
    'Line(((int)u16[0]).ToString("x", System.Globalization.CultureInfo.InvariantCulture) + " " + u16.Length.ToString(System.Globalization.CultureInfo.InvariantCulture));',
    '',
  ].join('\n');
