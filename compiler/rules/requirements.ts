...
const knownFlags: Record<string, Predicate> = {
  ...
  value_is_arbitrary_js_value: eq('throw.valueArbitrary', true),
  throw_expression_may_have_side_effects_or_throw: eq('throw.expressionMayAbrupt', true),
  sync_control_flow: eq('control.sync', true),
  catch_parameter_present: eq('catch.parameterPresent', true),
  lexical_scope_observable: eq('catch.lexicalScopeObservable', true),
  body_may_complete_abruptly: eq('try.bodyMayAbrupt', true),
  has_catch: eq('try.hasCatch', true),
  has_finally: eq('try.hasFinally', true),
  inside_function: eq('control.insideFunction', true),
  inside_iteration_or_labelled_statement: eq('control.insideIteration', true),
  loop_control_may_cross_try: eq('try.loopControlMayCross', true),
  finally_proven_to_complete_normally: eq('finally.normalOnly', true),
  thrown_js_values_use_wrapper_exception_representation: eq('throw.wrapper', 'JsException'),
  finally_may_complete_abruptly: eq('finally.mayAbrupt', true),
  nested_finally: eq('try.nestedFinally', true),
  lowering_requires_explicit_completion_representation: eq('completion.representation', 'JsCompletionSignal'),
  callee_may_throw: eq('callee.mayThrow', true),
...
const names: Record<string, string> = {
  ...
  context: 'control.context', catch_parameter_kind: 'catch.parameterKind', catch_binding: 'catch.binding', callee_kind: 'callee.kind',
